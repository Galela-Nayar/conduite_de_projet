...
export class UserProfilComponent {
    ...
    this.paramsSubscription = this.route.paramMap.subscribe((params) => {
      ...
      this.taskSubscription = this.observableService
        ...
        .subscribe((response) => {
          this.http
            .get<Utilisateur>(
              `http://localhost:8080/utilisateurs/user?id=${this.idUser}`
            )
  ...
  ngAfterViewChecked() {
    ...
    if (this.textareaUserName) {
      this.adjustTextareaUserName({
        target: this.textareaUserName!.nativeElement,
      });
  ...
  updateTaskNom() {
    this.http
      .get(
        `http://localhost:8080/utilisateurs/set_nom?id=${this.idUser}&param=${this.user.nom}`,
        { responseType: 'text' }
      )
      .subscribe((tacheData) => {
  ...
  editNom() {
    if (this.id == this.idUser) this.isEditingNom = true;
  }

  updateTaskPrenom() {
    this.http
      .get(
        `http://localhost:8080/utilisateurs/set_prenom?id=${this.idUser}&param=${this.user.prenom}`,
        { responseType: 'text' }
      )
      .subscribe((tacheData) => {
        this.updateUtilisateur();
  ...
  editPrenom() {
    if (this.id == this.idUser) this.isEditingPrenom = true;
  }

  updateTaskUserName() {
    this.http
      .get(
        `http://localhost:8080/utilisateurs/set_userName?id=${this.idUser}&param=${this.user.userName}`,
        { responseType: 'text' }
      )
      .subscribe(
        (tacheData) => {
  ...
  editUserName() {
    if (this.id == this.idUser) this.isEditingUserName = true;
  }

  updateTaskEmail() {
    this.http
      .get(
        `http://localhost:8080/utilisateurs/set_email?id=${this.idUser}&param=${this.user.email}`,
        { responseType: 'text' }
      )
      .subscribe(
        (tacheData) => {
  ...
  editEmail() {
    if (this.id == this.idUser) this.isEditingEmail = true;
  }

  updateTaskBio() {
    this.http
      .get(
        `http://localhost:8080/utilisateurs/set_bio?id=${this.idUser}&param=${this.user.bio}`,
        { responseType: 'text' }
      )
      .subscribe((tacheData) => {
        this.updateUtilisateur();
  ...
  editBio() {
    if (this.id == this.idUser) this.isEditingBio = true;
  }
  ...
  saveLogo() {
    if (this.selectedFile) {
      ...
      this.http
        .post(
          `http://localhost:8080/utilisateurs/set_logo?id=${this.idUser}`,
          formData,
          { responseType: 'text' }
        )
  ...
  openPasswordChangeDialog(): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      // Traitement après la fermeture de la boîte de dialogue (par exemple, mettre à jour le mot de passe)
      if (result) {
        // Mettez à jour le mot de passe ici
      }
    });
  }
}
